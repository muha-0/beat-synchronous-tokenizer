#!/usr/bin/env python3
"""
Training script for Tokenizer 3: Resample + CNN + Heart Rate Feature.

For sanity check on local GPU:
    python train_beat_hr.py --data_dir data/ptb-xl --epochs 3 --batch_size 4

For full training on A100/H100 (point to MIMIC):
    python train_beat_hr.py --data_dir /path/to/mimic --epochs 3 --batch_size 64
"""
...
from ecg_ssl.dataset import BeatHRECGDataset, beat_hr_collate_fn
from ecg_ssl.loss import masked_patch_mse_loss


def parse_args():
    parser = argparse.ArgumentParser(description="Tokenizer 3: Resample+CNN+HR ECG SSL")
    parser.add_argument("--data_dir", type=str, default="data/ptb-xl")
    parser.add_argument("--checkpoint_dir", type=str, default="checkpoints_beat_sync_hr")
...
def run_epoch(model, loader, device, mask_ratio, mask_span_len,
    ...
    """Training/validation loop for Tokenizer 3 model."""
    model.train(train)
...
def main():
    ...
    if len(step_history["step"]) > 0:
        ...
        plt.ylabel("Loss")
        plt.title("Tokenizer 3 (Resample+CNN+HR) SSL Training Monitor")
    ...
    if len(history) > 1:
        ...
        plt.xlabel("Epoch")
        plt.ylabel("Loss")
        plt.title("Tokenizer 3 (Resample+CNN+HR) Epoch Losses")
        plt.grid(True, alpha=0.3)
        plt.legend()
