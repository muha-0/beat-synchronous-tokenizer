#!/usr/bin/env python3
"""
Training script for Tokenizer 2: Adaptive Pooling CNN (no resampling).

For sanity check on local GPU:
    python train_beat_adaptive.py --data_dir data/ptb-xl --epochs 3 --batch_size 4

For full training on A100/H100 (point to MIMIC):
    python train_beat_adaptive.py --data_dir /path/to/mimic --epochs 3 --batch_size 64
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Tokenizer 2: Adaptive Pooling ECG SSL")
    parser.add_argument("--data_dir", type=str, default="data/ptb-xl")
    parser.add_argument("--checkpoint_dir", type=str, default="checkpoints_beat_sync_adaptive")
    ...
    parser.add_argument("--max_target_dim", type=int, default=8400,
                        help="Max reconstruction target size (12 leads * max beat samples)")
...
def run_epoch(model, loader, device, mask_ratio, mask_span_len,
              optimizer=None, scheduler=None, train=True,
              global_step=0, log_every=5, step_history=None):
    """Training/validation loop for Tokenizer 2 model."""
    model.train(train)
...
def main():
    ...
    if len(step_history["step"]) > 0:
        ...
        plt.ylabel("Loss")
        plt.title("Tokenizer 2 (Adaptive Pooling) SSL Training Monitor")
    ...
    if len(history) > 1:
        ...
        plt.xlabel("Epoch")
        plt.ylabel("Loss")
        plt.title("Tokenizer 2 (Adaptive Pooling) Epoch Losses")
        plt.grid(True, alpha=0.3)
        plt.legend()
